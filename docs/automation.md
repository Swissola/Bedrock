...
This hook runs with `--restricted` plus an explicit, minimal `--mcp-config`/`--strict-mcp-config` pair — confined to the checkout, with only the vault-write tools it actually needs, never broad unattended access. It also treats every changed filename as untrusted data rather than an instruction (a maliciously-named file shouldn't be able to redirect what the hook does), and verifies after every run — independent of what the run's own output claims — that only the intended doc was actually written, reverting anything else.
