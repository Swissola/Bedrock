...
**Testing this hook:** `tools/hook-templates/test-pre-commit.sh` (run with `bash tools/hook-templates/test-pre-commit.sh`). 13 assertions cover the no-staged-changes no-op, the no-scanner-installed warning (and that it's unthrottled), and the warn-versus-strict exit code branching. Unlike `post-merge`'s suite it needs no throwaway vault fixture, since this hook only reads staged content and never writes anything, just a throwaway repo and a stub `betterleaks` binary on `PATH`.
